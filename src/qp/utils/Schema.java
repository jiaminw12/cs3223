...
public class Schema implements Serializable {
	...
	/**
	 * The schema of resultant join operation, Not considered the elimination of
	 * duplicate Column
	 **/

	public Schema joinWith(Schema right) { // , Attribute leftAttr, Attribute
											// rightAttr){
	...
	/**
	 * To get schema due to result of project operation attrlist is the
	 * attirbuted that are projected
	 **/
	...
	public Object clone() {
		Vector newvec = new Vector();
		for (int i = 0; i < attset.size(); i++) {
			Attribute newatt = (Attribute) ((Attribute) attset.elementAt(i))
					.clone();
			newvec.add(newatt);
		}
		Schema newsche = new Schema(newvec);
