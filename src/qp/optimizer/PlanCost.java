...
public class PlanCost {
	...
	protected int getStatistics(Join node) {
		...
		/**
		 * get size of the tuple in output & correspondingly calculate buffer
		 * capacity, i.e., number of tuples per page
		 **/
		...
		int leftpages = (int) Math
				.ceil(((double) lefttuples) / (double) leftcapacity);
		int rightpages = (int) Math
				.ceil(((double) righttuples) / (double) rightcapacity);
	...
	protected int getStatistics(Select node) {
		...
		if (exprtype == Condition.EQUAL) {
			outtuples = (int) Math
					.ceil((double) intuples / (double) numdistinct);
		} else if (exprtype == Condition.NOTEQUAL) {
			outtuples = (int) Math.ceil(
					intuples - ((double) intuples / (double) numdistinct));
		} else {
			outtuples = (int) Math.ceil(0.5 * intuples);
		}

		/**
		 * Modify the number of distinct values of each attribute Assuming the
		 * values are distributed uniformly along entire relation
		 **/
		for (int i = 0; i < schema.getNumCols(); i++) {
			...
			int newvalue = (int) Math
					.ceil(((double) outtuples / (double) intuples) * oldvalue);
	...
	protected int getStatistics(Scan node) {
		...
		if (tokenizer.countTokens() != 1) {
			System.out
					.println("incorrect format of statastics file " + filename);
			System.exit(1);
		}
		...
		// System.out.println("Scan: tablename="+tablename+"pres
		// cost="+numpages+"total cost="+cost);
		return numtuples;
	}
