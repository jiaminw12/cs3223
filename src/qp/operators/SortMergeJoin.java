...
public class SortMergeJoin extends Join {
	...
	ObjectInputStream rightSorted; // File pointer to the right hand materialized sorted file
	ObjectInputStream leftSorted; // File pointer to the right hand materialized sorted file
	...
	public boolean open() {
		...
		try{
			...
			leftSortedFileName = "merged_left_pass"+numPassLeft+"_subRound0";
			leftSorted =  new ObjectInputStream(new FileInputStream(leftSortedFileName));
			
			rightSortedFileName = "merged_right_pass"+numPassRight+"_subRound0";
			rightSorted =  new ObjectInputStream(new FileInputStream(rightSortedFileName));
		...
		if(!goToSubsetJoin){
			try{
				...
				while(!outbatch.isFull()){
					int result = Tuple.compareTuples(l, r, leftindex,rightindex);
					if(result>0){ /////////l>r , r= next tuple in right
						if(rE==(blockRight.get(rB).size()-1)){ ////case1: end of one right page
							rB++;
							rE=0;
						}else if (rB==(blockRight.size()-1)){ //// case2: end of one whole right block
							blockRight = new ArrayList<Batch>();
							for(int i = 0;i<halfBlock;i++){// read in another block from sorted file
								if(rightSorted.available()!=0){
									blockRight.add((Batch)rightSorted.readObject());
								}else{
									break;
					...
					}else if(result<0){ /////////l<r , l= next tuple in left
						if(lE==(blockLeft.get(lB).size()-1)){ //// case1: end of one left page
							lB++;
							lE=0;
						}else if (lB==(blockLeft.size()-1)){ //// case2: end of one whole left block
							blockLeft = new ArrayList<Batch>();
							for(int i = 0;i<halfBlock;i++){// read in another block from sorted file
								if(leftSorted.available()!=0){
									blockLeft.add((Batch)leftSorted.readObject());
						...
						boolean rOver = false; // represent right same tuple exceed current block
						boolean lOver = false; // represent left same tuple exceed current block
						
						Tuple r2 = blockRight.get(blockRight.size()-1)  // get last element in last page of rightblock
								.elementAt(blockRight.get(halfBlock-1).size()-1);
						int compR = Tuple.compareTuples(r, r2, rightindex);
						
						Tuple l2 = blockLeft.get(blockLeft.size()-1)  // get last element in last page of leftblock
								.elementAt(blockLeft.get(halfBlock-1).size()-1);
						...
						if(rOver && !lOver){////case1: only tuples in right block overflows
							//write the left pages that contains tuple to a left subset file
							String subset = "leftSubset";
							ObjectOutputStream sub = new ObjectOutputStream(new FileOutputStream(subset));
							int offsetEqualB=-1;
							int offsetDiffB=-1; 
							int offsetEqualE=-1;
							int offsetDiffE=-1; 
							outerloop:
							for(int i = lB;i<blockLeft.size();i++){ 
								for(int j = 0;j<blockLeft.get(i).size();j++){ 
									Tuple t3 = blockLeft.get(i).elementAt(j);
									int compRI = Tuple.compareTuples(l,t3, leftindex);
									if(compRI==0){
										offsetEqualB=i;
										offsetEqualE=j;
									}else{
							...
							// write the pages containing same tuple into a right subset file
							...
							////case2: only tuples in left block overflows
								//write the left pages that contains tuple l a left subset file
								...
								int offsetDiffE=-1; 
								outerloop:
								for(int i = rB;i<blockRight.size();i++){ 
									for(int j = 0;j<blockRight.get(i).size();j++){ 
										Tuple t3 = blockRight.get(i).elementAt(j);
										int compRI = Tuple.compareTuples(l,t3, rightindex);
										if(compRI==0){
											offsetEqualB=i;
											offsetEqualE=j;
										}else{
								...
								// write the pages containing same tuple into subset file
								for(int i = 0;i<=offsetEqualB;i++){ 
						...
						////case4:  tuples in both blocks never overflow
							...
							///r = next tuple in right block to compare with fixed l
							int offsetEqualBR=-1;
							int offsetDiffBR=-1; 
							int offsetEqualER=-1;
							int offsetDiffER=-1;
							...
							for(int i = rB;i<blockRight.size();i++){ 
								for(int j = 0;j<blockRight.get(i).size();j++){ 
									...
									if(r2.checkJoin(l, leftindex, rightindex)){
							...
							///l = next tuple in left block to compare with fixed r
							int offsetEqualBL=-1;
							int offsetDiffBL=-1; 
							int offsetEqualEL=-1;
							int offsetDiffEL=-1;
							...
							for(int i = lB;i<blockLeft.size();i++){ 
								for(int j = 0;j<blockLeft.get(i).size();j++){ 
									...
									if(l2.checkJoin(r, leftindex, rightindex)){
				...
				System.out.println("error in reading object from sorted: "+error);
			}catch (IOException io){
				System.out.println("error in reading object from sorted: "+io);
			}
		...
		try {
			...
			while (!outbatch.isFull()) {
				...
				if (lcurs == 0 && eosr == true) {
					 ...
					 * of right table
					 **/
						inR = new ObjectInputStream(new FileInputStream("rightSubset"));
						eosr = false;		
			...
			System.err.println("NestedJoin:error in reading the subset file: "+io);
		}catch (ClassNotFoundException e) {
			System.err.println("NestedJoin:error in reading the subset file: "+e);
		}
		return outbatch;
	...
	private void formSubset(String setName,Tuple t){
		...
		try{
			...
			while(comp==0){
				// write current page until the last page in the current block into subset file
				...
				Tuple t2 = blk.get(blk.size()-1)  // get last element in last page of block
	...
	public int externalSort(Operator node, String nameOfFile, int indexOfAttr) throws ClassNotFoundException{
		Batch unsortedRun;
		int passCount = 0;
		int numOfRun=0;
		...
		//	}
			...
			ObjectInputStream inMerged = null ;// initialize source file for input runs
			int sizeOfOutputBatch = Batch.getPageSize()/node.schema.getTupleSize(); //size of output page
			ArrayList<Integer> subRoundFileIndex = findNumOfPass(numOfRun);//subRound means num of blocks that one pass requires
			...
			while(curPass<passCount){
				...
				////determine the source file of unmerged runs for the initial pass
				if(curPass==0){
					inMerged = new ObjectInputStream(new FileInputStream(outSortedFile));
				}
				
				////repeatedly read sorted runs into block buffer 
				for (int subRound=0;subRound<subRoundFileIndex.get(curPass);subRound++){
					
					//// create the destination of merged runs, one block of pages form one file
					
					int outPassIndex = curPass+1;
					int outSubRoundIndex = subRound;
					String partialOutMergedFile = "merged"+nameOfFile+"_pass"+outPassIndex+"_subRound"+outSubRoundIndex;;
					ObjectOutputStream partialOutMerged = new ObjectOutputStream(new FileOutputStream(partialOutMergedFile));
					
					
					//// for each page in each block, read tuples to the page until all tuples read
					
					if(curPass>0){ // other than the initial pass, the input source are multiple files
						ObjectInputStream[] partialInMerged = new ObjectInputStream[numBuff-1];// give each page a pointer to respect input file
						int validPage = 0; //count number of non-empty page in the block
						int pageNo;
						for(pageNo = 0;pageNo<(numBuff-1);pageNo++){ 
							int inPassIndex = curPass;
							int inSubRoundIndex = subRound+pageNo;
							String partialInMergedFile = "merged"+nameOfFile+"_pass"+inPassIndex+"_subRound"+inSubRoundIndex;
							File f = new File(partialInMergedFile);
							if(!f.exists()){
								break;
							}else{
								validPage++;
								partialInMerged[pageNo] = new ObjectInputStream(new FileInputStream(partialInMergedFile));
							}
						}
						
						 
						boolean runHasMoreTuple = true; // flag to show there is more tuple not read into a page
						while(runHasMoreTuple){
							block = new ArrayList<Batch>();// represents the (numBuff-1) pages storing sorted runs for merge
							runHasMoreTuple = false;
							for(int i = 0;i<validPage;i++){ 
								if(partialInMerged[i].available()!=0){
									runHasMoreTuple = true;
									block.add((Batch)partialInMerged[i].readObject());
								}								
							}
							if(runHasMoreTuple) {
	...
	private void mergeOneBlock(Operator node, String nameOfFile, int indexOfAttr,
			ObjectOutputStream partialOutMerged, int sizeOfOutputBatch) {
		
		Batch outputBatch = new Batch(sizeOfOutputBatch); //the output page for merged sorted runs 
		...
		while(blockHasMoreTuple){
			...
			for(int j = 0;j<(numBuff-1);j++){ // get the minimum among current element(0)
				if(block.get(j)!=null){
					blockHasMoreTuple = true;
					Tuple toCheck = block.get(j).elementAt(0);
					Object toCheckVal = block.get(j).elementAt(0).dataAt(indexOfAttr);
					if(compareElement(toCheckVal,minVal)<0){ // if current element is smaller than min
						min = toCheck;
						index = j;
					}
				}
			...
			if (outputBatch.size() == sizeOfOutputBatch
				...
				try{
					...
					System.out.println("error in writing outputbatch object: "+io);
				}
					outputBatch = new Batch(sizeOfOutputBatch);
			}else if(blockHasMoreTuple){
				outputBatch.add(min);
				block.get(index).remove(0); // remove current min from the block buffer
			}
		}
	}
	     ...
	     while ( flag ){
	            ...
	            for(int r = 0;r< run.size();r++ ){
	                   if (compareElement(run.elementAt(r).dataAt(indexOfAttr),
	                		   run.elementAt(r+1).dataAt(indexOfAttr))>0 )   // ascending sort
	                   {
