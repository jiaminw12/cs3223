...
public class IndexNestedJoin extends Join {
	...
	TreeMap<Object, Vector<Tuple>> rightMap; // a hash map to store tuples under
												// same value of an attr
	Vector<Tuple> valEqual; // to store vectors matches the given attr in hash
							// map
	 ...
	 * hand side Opens the connections
	 **/
	...
	public Batch next() {
		...
		while (!outbatch.isFull()) {
			...
			for (i = lcurs; i < leftbatch.size(); i++) {
				...
				//// a new valEqual that matches the current left tuple to be
				//// found
